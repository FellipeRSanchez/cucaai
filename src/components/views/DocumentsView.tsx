...
import { FileText, Trash2, Clock, CheckCircle, Loader2, Folder } from 'lucide-react';
...
export function DocumentsView() {
  ...
  const fetchProjects = useCallback(async () => {
    const { data } = await supabase
      .schema('cuca')
      .from('projetos')
      .select('pro_id, pro_nome, pro_emoji');
  ...
  const getProject = (projetoId: string | null) => {
    if (!projetoId) return null;
    return projects.find(p => p.pro_id === projetoId);
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {isLoading ? (
          ...
          <div className="grid gap-4">
            {docs.map((doc) => {
              const project = getProject(doc.doc_projeto_id);
              return (
                <div key={doc.doc_id} className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-4 flex items-center justify-between group hover:border-zinc-700 transition-colors">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        ...
                        {project && (
                          <span className="text-xs text-zinc-500 flex items-center gap-1">
                            <Folder size={12} />
                            {project.pro_emoji} {project.pro_nome}
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  <button
                    ...
                    className="p-2 text-zinc-500 hover:text-red-400 hover:bg-red-500/10 rounded-md transition-all opacity-0 group-hover:opacity-100"
                  >
                    <Trash2 size={18} />
                  </button>
                </div>
